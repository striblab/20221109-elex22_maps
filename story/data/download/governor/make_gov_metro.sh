...
precincts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null 'https://electionresultsfiles.sos.state.mn.us/20181106/allracesbyprecinct.txt') > gov-metro.csv &&

csv2json -s ";" gov-metro.csv | ndjson-cat | \
  ndjson-split | \
  ndjson-filter "d.office_id == \"$OFFICE_ID\"" > $DISTRICT_STR.tmp.ndjson &&

echo "Downloading precinct maps ..." &&
  shp2json metro.shp | \
  mapshaper - -quiet -proj longlat from=metro.prj -o ./metro.json format=geojson && \
  cat metro.json | \
...
topo2geo precincts=$DISTRICT_STR-results-metro.json < precincts-final.json &&

echo "Creating SVG ..." &&
mapshaper $DISTRICT_STR-results-metro.json \
  -quiet \
