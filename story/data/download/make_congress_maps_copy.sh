...
MAPSHAPER_CATEGORIES="COLLIN C. PETERSON,LEE BYBERG"
...
precincts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null "https://electionresults.sos.state.mn.us/Results/MediaFile_Archive/Index?erselectionId=1&mediafileid=29") > house.csv && 

csv2json -s ";" house.csv | ndjson-cat | \
  ndjson-split | \
