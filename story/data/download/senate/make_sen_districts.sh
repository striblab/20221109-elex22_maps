...
districts_voting;votes;votes_pct;votes_office" | \
  cat - <(wget -O - -o /dev/null 'https://electionresultsfiles.sos.state.mn.us/20221108/stsenate.txt') > sen-district.csv &&
...
echo "Downloading precinct maps ..." &&
  shp2json senate2022.shp | \
  mapshaper - -quiet -proj longlat from=senate2022.prj -o ./senate2022.json format=geojson && \
  cat senate2022.json | \
